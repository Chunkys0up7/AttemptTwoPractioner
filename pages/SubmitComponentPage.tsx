...
import { UploadIcon, CheckCircleIcon, XCircleIcon, PlusCircleIcon, TrashIcon } from '../icons';
...
interface TypeSpecificFormProps {
  data: AIComponent['typeSpecificData'];
  onChange: (field: keyof NonNullable<AIComponent['typeSpecificData']>, value: any) => void;
  setFormError: (field: keyof NonNullable<AIComponent['typeSpecificData']>, error: string | undefined) => void;
  errors: Partial<Record<keyof NonNullable<AIComponent['typeSpecificData']>, string>>;
}
...
const CodeEditorForm: React.FC<TypeSpecificFormProps & { language: string }> = ({ data, onChange, errors, language }) => (
  <div>
    ...
    <textarea
      ...
      rows={10}
      className="w-full px-3 py-2 border border-neutral-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm font-mono bg-white text-neutral-900 placeholder-neutral-500"
      placeholder={`Enter your ${language.toLowerCase()} script here...`}
    />
  ...
  return (
    <div>
            ...
            <div className="p-3 border-b border-neutral-200">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <div>
                        <select 
                            value={cell.type} 
                            onChange={(e) => changeCellType(cell.id, e.target.value as 'code' | 'markdown')}
                ...
                rows={cell.type === 'code' ? 5 : 3}
                className="w-full px-2 py-1.5 border border-neutral-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm font-mono text-xs bg-white text-neutral-900 placeholder-neutral-500"
      ...
      <div className="flex space-x-2 mt-2">
        <Button type="button" variant="outline" size="sm" onClick={() => addCell('code')} leftIcon={<PlusCircleIcon className="w-4 h-4" />}>Add Code Cell</Button>
        <Button type="button" variant="outline" size="sm" onClick={() => addCell('markdown')} leftIcon={<PlusCircleIcon className="w-4 h-4" />}>Add Markdown Cell</Button>
      </div>
      {errors?.notebookCells && <p className="mt-1 text-xs text-red-500">{errors.notebookCells}</p>}
    </div>
  );
};
    ...
    return (
        <div className="space-y-4">
            <FormRow label="Git Repository URL" htmlFor="gitRepoUrl">
                <input type="url" name="gitRepoUrl" id="gitRepoUrl" value={streamlitData.gitRepoUrl || ''} onChange={handleChange}
                    className="w-full px-3 py-2 border border-neutral-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm bg-white text-neutral-900 placeholder-neutral-500" placeholder="https://github.com/user/repo.git"/>
            </FormRow>
            <FormRow label="Main Script Path (e.g., app.py)" htmlFor="mainScriptPath">
                <input type="text" name="mainScriptPath" id="mainScriptPath" value={streamlitData.mainScriptPath || ''} onChange={handleChange}
                    className="w-full px-3 py-2 border border-neutral-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm bg-white text-neutral-900 placeholder-neutral-500" placeholder="app.py"/>
            </FormRow>
            <FormRow label="Requirements (content of requirements.txt)" htmlFor="requirements">
                <textarea name="requirements" id="requirements" value={streamlitData.requirements || ''} onChange={handleChange} rows={5}
                    className="w-full px-3 py-2 border border-neutral-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm font-mono bg-white text-neutral-900 placeholder-neutral-500" placeholder="streamlit\npandas\n..." />
            </FormRow>
...
const MCPEditorForm: React.FC<TypeSpecificFormProps> = ({ data, onChange, errors }) => (
  <div>
    ...
    <textarea
      ...
      onChange={(e) => onChange('mcpConfiguration', e.target.value)}
      rows={10}
      className="w-full px-3 py-2 border border-neutral-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm font-mono bg-white text-neutral-900 placeholder-neutral-500"
      placeholder="Enter MCP definition here..."
    />
...
const SubmitComponentPage: React.FC = () => {
  ...
  const [typeSpecificData, setTypeSpecificData] = useState<AIComponent['typeSpecificData']>({});
  
  const [errors, setErrors] = useState<Partial<Record<keyof CommonFormData | keyof AIComponent['typeSpecificData'], string>>>({});
  ...
  const handleTypeSpecificChange = (field: keyof NonNullable<AIComponent['typeSpecificData']>, value: any) => {
    setTypeSpecificData(prev => ({ ...prev, [field]: value }));
    setErrors(prev => ({ ...prev, [field]: undefined }));
  };

  const setTypeSpecificError = (field: keyof NonNullable<AIComponent['typeSpecificData']>, error: string | undefined) => {
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof CommonFormData | keyof AIComponent['typeSpecificData'], string>> = {};
...
const FormRow: React.FC<{ label: string; htmlFor?: string; error?: string; children: React.ReactNode; required?: boolean }> = 
({ label, htmlFor, error, children, required }) => (
  <div>
