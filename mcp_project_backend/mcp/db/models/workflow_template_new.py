from sqlalchemy import Column, Integer, String, JSON, ForeignKey, DateTime, Boolean, UUID
...
class TemplateShare(Base):
    ...
    id = Column(Integer, primary_key=True)
    template_id = Column(Integer, ForeignKey('workflow_templates.id'), nullable=False)
    user_id = Column(String(36), ForeignKey('users.id'), nullable=False)  # Stores UUID as string
